...
digizeit <- read.csv("placebo.csv", header=FALSE)
#digizeit <- read.csv("bevacizumab.csv", header=FALSE)
t.S<-digizeit[,1]
S<-digizeit[,2]
...
IPD.Ohtsu <- rbind(IPD.Ohtsu.placebo, IPD.Ohtsu.bevacizumab)
...
fwrite(IPD.Ohtsu, file = "IPD.Ohtsu.csv", sep = ",")
