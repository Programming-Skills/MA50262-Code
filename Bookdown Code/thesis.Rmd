...
Jimenez et al. gives a definition of the Weighted Log Rank Test (WLRT): 
...
--------------- -------------------- --------------- ----------------------- --------------------
      ...
      0                  1               Late                $1-S(t-)$                -
------------------------------------------------------------------------------------------------
Table: (\#tab:inher) Weighted Log Rank Test Weight Functions

## Weights for the WLRT
...
```{r, echo=FALSE}
# FH(1,0) S(t-) vs t (Prentice-Wilcoxon)
plot(x = S$time, y = S$surv,
     main = "FH(1,0)  S(t-) vs Time (Prentice-Wilcoxon)", 
     xlab = "Time",
     ylab = "1-S(t-)",
     col = "blue")
```
...
The Schoenfeld residuals are calculated by subtracting the expected value of the model from the scaled covariate value of the $i^{th}$ observed event. (Please see 0.1 Diagnosics for more information) and Adam O gives an explanation of the the pattern of the Schoenfeld residuals: for this model the dummy covariate "Gefitinib" which takes 1 if the patient is on the gifitinib treatment arm or 0 if the patient is on the carboplatin–paclitaxel treatment arm. If there is zero association and subjects are balanced in the trial (equal numbers in both arms), it would follow that the covariates expection would be 0.5. Alternatively, it could be said that a subject would equally likely to experience the event (death) in either treatment arm at any time point. If this were the case we would see no variability in the Schoenfeld residuals (a definite trend). 
...
The Cox Proportional Hazards Model can provide an estimation of the hazard ratio (HR) (A measure of how likely a particular event is to occur in one group cpmpared to another group over time). 
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the gefitinib tratment is ((1-0.76)*100) 24% less likely to die than those patients on the carboplatin dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 0.66 and 0.86 and the p-value for the test that the gefitinib coefficient is actually zero is less than 0.001.
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the nivolumab treatment is ((1-0.76)*100) 24% less likely to die than those patients on the docetaxel dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 0.62, 0.92 and the p-value for the test that the nivolumab coefficient is actually zero is less than 0.005.
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the gefitinib tratment is ((1-2.80)*100) 180% more likely to die than those patients on the carboplatin dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 2.02 and 3.89 and the p-value for the test that the gefitinib coefficient is actually zero is less than 0.001.
...
The AVAGAST study which was titled "Bevacizumab in Combination With Chemotherapy As
First-Line Therapy in Advanced Gastric Cancer" was an randomised international study that compared a placebo to a combination of bevacizumab to capecitabine-cisplatin in the treatment of advanced gastric cancer.

The study participants recieved a placebo or bevacizumab 7.5 mg/kg on the first day plus a dose of capecitabine 1,000 mg/m2 twice a day for 14 days every three weeks. For patients not able to take oral medications Fluorouracil was given. Futhermore, six cycles of Cisplatin were administered; bevacizumab and cisplatin were given until the toxity was too high or the patient suffered disease progression.

The primary endpoint of the trial was the progression free survival of the subject. 

From a visual inspection it is seen that the bevacizumab hazard is less that the placebo throughout the course of the trial but the hazard is the same as the control arm by the end of the trial, point to the suposition that the treatment is ineffective.
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the bevacizumab tratment is ((1-0.87)*100) 13% less likely to die than those patients on the placebo dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 0.73 and 1.04 and the p-value for the test that the bevacizumab coefficient is actually zero is less than 0.12.
...
patients with unresectable or metastatic melanoma" was a mutlinational double-blind phase 3 trial that recruited patients suffering from previously untreated or treated advanced skin cancer and who had not previously been treated with immune checkpoint or BRAF inhibitors. The subjects were randomly assigned (all participants were masked to which treatment arm they were assigned to) to recieve either ipilimumab 10 mg/kg or 3 mg/kg, which was delivered intravenously every 3 weeks for 90 minutes. Furthermore, patients were stratified by disease progression status.
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the ipilimumab 10 mg/kg treatment is ((1-0.85)*100) 15% less likely to die than those patients on the ipilimumab 3 mg/kg dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 0.72 and 1.01 and the p-value for the test that the ipilimumab 10 mg/kg coefficient is actually zero is less than 0.061.
...
The OAK study 2017: was titled "Atezolizumab versus docetaxel in patients with previously treated non-small-cell lung cancer" investigated if randomly assigned cancer patients aged 18 and above and had not recieved some forms of prior treatment including docetaxel to recieve either docetaxel 75 mg/m2 every 3 weeks or an intravenous dose of 1200mg of atezolizumab

The primary endpoint of the trial was the progression free survival of the subject in the intend to treat population and and PD-L1-expression population on their cancers. 
...
From a visual inspection it can be seen that the atezolizumab and docetaxel hazard are the same for the first three months of the study. After three months the treatment arms separate and atezolizumab is less than and docetaxel hazard for the remainder of the treatment period.
...
By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the atezolizumab treatment is ((1-0.72)*100) 28% less likely to die than those patients on the docetaxel dose if the assumptions of the model are satified.  

Furthermore, the model is 95% confident that the true HR lies between 0.61 and 0.85 and the p-value for the test that the atezolizumab coefficient is actually zero is less than 0.001.
