...
This paper will now proceed to give a concise outline of censoring, the hazard function, the Cox model and some of the most important characteristics of this approach.
<br/><br/>
...
By letting $\hat{\theta}$ equal to $\exp \left[\sum_{i=1}^{p} \hat{\beta}_{i}\left(X_{i}^{*}-X_{i}\right)\right]$ it follows that $\frac{\hat{h}\left(t, \mathbf{X}^{*}\right)}{\hat{h}(t, \mathbf{X})}=\hat{\theta}$ and so $\hat{h}\left(t, X^{*}\right)=\hat{\theta} \hat{h}(t, X)$ where the constant of proportionality $\hat{\theta}$ is not dependent on $\textit{t}$. Therefore, one subjects hazard function is in proportion to another subject's hazard. 
...
An alternative way to understand the weights is that they are the chance of choosing an individual from the risk set at time $t$.
...
As the LRT is a non-parametric test, it performs well when the non-proportional hazards (NPH) assumption is not violated. However, when this assumption is broken the LRT is not sufficiently powerful. Furthermore, if the proportional hazards assumption is broken, it makes the hazard ratio (can be thought of as an average hazard ratio, which is subject to the overall follow-up of study participants) difficult to interpret. 
...
If the NPH assumption were not violated this project would expect to see a hazard ratio of one throughout the treatment time period $t$. This suggests events will be evenly spaced across $t$.
...
An approach developed by Guyot et al. 2012 that can be used with data that is left or right censored, makes use of an iterative algorithm. From this recreated IPD the survival curves can be reconstructed. In contrast to the approach put forward by Hoyle et al. the estimated IPD is precise survival data and not interval survival data. However, an assumption of Guyot et al. is that the rate of censoring is constant within a given time interval. 

A concise explanation of the Guyot et al. algorithm is now given: 

```{r, echo=FALSE, fig.cap="Guyot Algorithm Flowchart"}
knitr::include_graphics(path = "figure/guyot.flowchart.PNG")
```

The  initial estimate of the number of censored subjects is obtained for the $i^{th}$ interval. From this initial estimate, a calculation of the number of censored subjects that occurred between the extracted KM co-ordinate $k$ and $k+1$ is made. Now this calculation is used to estimate the number of events that occur at every extracted KM co-ordinate $k$ and the numbers of patients at risk at $k+1$. This information is used to compare the estimate of the number of patients at risk at the start of the interval $i$ to the reported number at risk at the beginning of the interval $i$, and if the numbers are not equal the process repeats itself until the two numbers are equal and this matches the previous iteration. Conversely, in the situation where, the total number of the reported events is greater than the estimate of the number of events the process repeats itself until the two numbers equal from the previous iteration.

Guyot et al. evaluated the accuracy of their approach by comparing the findings to the reported statistics in four papers that they selected to recreate, choosing not to carry any simulations. These included 22 probabilities of survival, 7 times of median survival, 6 hazard ratios and 4 log hazard ratio standard errors. The researchers discovered that their approach was highly accurate for both median survival times and survival probabilities and the accuracy was acceptable when the total number of events or the number of patients at risk was given. 
...
To implement the Guyot et al. algorithm and reconstruct the IPD for a published KM curve several steps must be undertaken. Noting that it is imperative that the number of patients at risk and/or the total number of events must be known to apply this method.

A number of researchers have made use of the Guyot et al. algorithm in their work, this project will be using the method used by Satagopan et al. and some adjustments proposed by Magirr D and Biao et al.
...
## Mok et al. 2009 IPASS Study: Crossing Hazards {#mok.a}
...
Previously untreated East Asian patients who were suffering from non–small-cell lung cancer (advanced pulmonary adenocarcinoma) and were either former light smokers or non-smokers were assigned randomly to be treated with either a daily dose of 250mg of gefitinib (609 subjects) or a dose of carboplatin of 5 to 6mg per milliliter per minute and a dose a 200mg per meter squared of body-surface area (608 subjects). 

The primary endpoint of the trial was the progression free survival of the subject. 

From a visual inspection it is apparent that the experimental treatment begins with a greater hazard but there is a crossing of the treatment arms at around six months after that time the control arm presents the higher hazard, for overall patient outcome.
...
The Schoenfeld residuals are calculated by subtracting the expected value of the model from the scaled covariate value of the $i^{th}$ observed event. (Please see [0.1 Model Diagnostics](#diag) for more information) and Adam O gives an explanation of the pattern of the Schoenfeld residuals: for this model the dummy covariate "Gefitinib" which takes 1 if the patient is on the gefitinib treatment arm or 0 if the patient is on the carboplatin–paclitaxel treatment arm. If there is zero association and subjects are balanced in the trial (equal numbers in both arms), it would follow that the covariates expectation would be 0.5. Alternatively, it could be said that a subject would equally likely to experience the event (death) in either treatment arm at any time point. If this were the case, no variability in the Schoenfeld residuals (a definite trend) would be witnessed. 

Moore D explains that the smooth (loess) curve's shape gives a visual representation of an estimate of the difference parameter as function of $t$. Additionally, a 95% confidence interval is given by the dashed lines.

This project will now check proportional hazards by using the Schoenfeld test for proportion hazards:
...
Due to the very large test statistic (chisq = 113) value and small p value (p = 2e-16), there is significant evidence of non-proportional hazards. 

In Fig 3.1 the estimate shows a steady decline in the hazard until approximately the seven-month mark, after that there is a slow constant increase in hazard. This reflects the effect seen in the KM curve, where the gefitinib treatment is at first detrimental to the patient compared to those treated with carboplatin, but at around seven months starts to give benefit when compared to the carboplatin.

From this analysis, there is clear evidence that the proportional hazards assumption has been violated and the results of the Cox proportional hazards model could be biased, giving the rationale to examine alternative methods to the overcome this issue.

<br>

```{r echo=FALSE, fig.cap="Cox Proportional Hazards Model", message=FALSE, warning=FALSE}
source("Libraries.R")
...
The baseline hazard is unspecified in the Cox model. Therefore, the researcher cannot estimate the survival with the Cox Proportional Hazards Model. Furthermore, because the model does not estimate the intercept it is not possible to estimate the hazard or make an estimation of the survival function.

The Cox Proportional Hazards Model can provide an estimation of the hazard ratio (HR) (A measure of how likely an event is to occur in one group compared to another group over time). 

The reported HR for the Cox Model in the gefitinib treatment arm is: 0.76 (carboplatin-paclitaxel is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the gefitinib is 0.76 times as likely to die as a patient who is on carboplatin treatment. Conversely a patient on the standard of care is 1.2438 times as likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the gefitinib treatment is ((1-0.76)*100) 24% less likely to die than those patients on the carboplatin dose if the assumptions of the model are satisfied.  

Furthermore, the model is 95% confident that the true HR lies between 0.66 and 0.86 and the p-value for the test that the gefitinib coefficient is zero is less than 0.001.
...
At a given instant in time a patient receiving the Gefitinib treatment is 0.965 times as likely to die as a patient who is on the Carboplatin-Paclitaxel treatment. The 95% confidence interval was (0.844, 1.104) with p value 0.605.
...
Table \@ref(tab:wlr) gives information on the $G^{\rho, \gamma}$ collection of weights used in the WLRT for this project. Additionally, the study data has been plotted using the weight functions of this test. FH(0,0), FH(1,0), FH(1,1), FH(0,1) to give a visual representation of the various weights used in this research.

The WLRT results shown in table 3.3, reveal that all the p-values aside from FH(1,0) are statistically significant (a weight of FH(1,0) would signify more events occurring in the latter period of the study, and as per the KM curves this is clearly not the case). It would be expected that FH(0,1) would be significant as it is apparent that more events are likely to occur in the early portion of the treatment and potentially FH(1,1) the middle. However, there is little intuitive reason why F(0,0) is significant was there is a clear violation of the PH assumption. 
...
The Brehmer et al. 2015 study titled: "Nivolumab versus Docetaxel in Advanced Nonsquamous Non–Small-Cell Lung Cancer" was an open labelled randomised phase 3 trial which was global in scope. Patients who had previously been treated with a dose of 75mg docetaxel per square meter of body-area every each 3 week period or platinum-based doublet chemotherapy before or after progression of nonsquamous non–small-cell lung cancer (NSCLC) were assigned a dose of nivolumab of 3mg per kilogram of body-weight every two  weeks. 

The trial's primary end point was overall survival.

*Nivolumab is a "a fully human IgG4 programmed death 1 (PD-1) immune-checkpoint–inhibitor antibody, disrupts PD-1–mediated signaling and may restore antitumor immunity. 

From a visual inspection it is apparent that the experimental treatment begins with a greater hazard but there is a crossing of the treatment arms at around seven months after that time the control arm presents the higher hazard, for overall patient survival. survival.
...
Due to the large test statistic (chisq = 9.56) value and small p value (p = 0.002), there is significant evidence of non-proportional hazards. 

In Fig 3.2 the estimate shows a slow steady decline in the hazard until approximately the ten- month mark, after that there is a rapid increase in hazard. This reflects the effect seen in the KM curve, where the nivolumab treatment is at first detrimental to the patient compared to those treated with docetaxel, but at around ten months starts to give benefit when compared to the docetaxel arm.
...
The reported HR for the Cox Model in the nivolumab treatment arm is: 0.76 (docetaxel is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the nivolumab is 0.76 times as likely to die as a patient who is on docetaxel treatment. Conversely a patient on the standard of care is 1.2436 times as likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the nivolumab treatment is ((1-0.76)*100) 24% less likely to die than those patients on the docetaxel dose if the assumptions of the model are satisfied.  
Furthermore, the model is 95% confident that the true HR lies between 0.62, 0.92 and the p-value for the test that the nivolumab coefficient is zero is less than 0.005.
...
At a given instant in time a patient receiving the Nivolumab treatment is 0.846 times as likely to die as a patient who is on the Docetaxel treatment. The 95% confidence interval was (0.691, 1.0348) with p value 0.104.
...
The WLRT results shown in table 3., reveal that all the p-values are statistically significant. It would be expected that FH(0,1) would be significant (this is the most significant p value) as it is apparent that more events are likely to occur in the early portion of the treatment and potentially FH(1,1) the middle. However, there is little intuitive reason why F(0,0) or F(1,0) are significant was there is a clear violation of the PH assumption, so the standard log rank test would not be the most powerful test, and that the majority of events would occur early in the trial.
...
## Mok et al. 2009 IPASS Study: Diminishing Treatment Effect
...
From a visual inspection it is apparent that the experimental treatment begins with a greater hazard but there is a crossing of the treatment arms at around six months after that time the control arm presents the higher hazard, for EGFR-Mutation–Negative patient group outcome.
...
Due to the large test statistic (chisq = 17) value and small p value (p = 3.7e-05), there is significant evidence of non-proportional hazards. 

In Fig 3.1 the estimate shows a steady decline in the hazard until approximately the ten-month mark. This reflects the effect seen in the KM curve, where the gefitinib treatment is detrimental to the patient compared to those treated with carboplatin, giving no benefit to patients within this group when compared to the standard of care.
...
The reported HR for the Cox Model in the gefitinib treatment arm is: 2.80 (carboplatin-paclitaxel is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the gefitinib is 2.80 times as likely to die as a patient who is on carboplatin treatment. Conversely a patient on the standard of care is 1.8 times less likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the gefitinib treatment is ((1-2.80)*100) 180% more likely to die than those patients on the carboplatin dose if the assumptions of the model are satisfied.  

Furthermore, the model is 95% confident that the true HR lies between 2.02 and 3.89 and the p-value for the test that the gefitinib coefficient is zero is less than 0.001.
...
In this study the increase in the censoring weights are large during the treatment. However, it should be noted that the normalised total weight range is [0, 2]. 
...
The WLRT results shown in table 3.3, reveal that all the p-values are statistically significant. It would be expected that FH(1,1) would be significant (this is one of the most significant  p values) as it is apparent that more events are likely to occur in the early and late portions of the trial. However, there is little intuitive reason why F(0,0) is significant was there is a clear violation of the PH assumption. Furthermore, the significant p values for FH(0,1) and FH(1,0) has little intuitive explanation as this would imply that events were more likely to occur at the beginning and end of the trial, and from the KM curves it can be seen that this is not the case.
...
As the package nphsim uses one sided p values so for this data set, this project will use the p value from the control arm and then take 1/exp(coef) to calculate the HR. The result of 1/0.25136 is 3.978358.
...
## Ohtsu et al. 2011 AVAGAST Study: Diminishing Treatment Effect
...
The AVAGAST study was titled "Bevacizumab in Combination with Chemotherapy As
First-Line Therapy in Advanced Gastric Cancer" was a randomised international study that compared a placebo to a combination of bevacizumab to capecitabine-cisplatin in the treatment of advanced gastric cancer.

The study participants received  a placebo or bevacizumab 7.5 mg/kg on the first day plus a dose of capecitabine 1,000 mg/m2 twice a day for 14 days every three weeks. For patients not able to take oral medications Fluorouracil was given. Futhermore, six cycles of Cisplatin were administered; bevacizumab and cisplatin were given until the toxicity was too high or the patient suffered disease progression.

The primary endpoint of the trial was the progression free survival of the subject. 

From a visual inspection it is seen that the bevacizumab hazard is less that the placebo throughout the course of the trial but the hazard is the same as the control arm by the end of the trial, point to the supposition that the treatment is ineffective.
...
Due to the large test statistic (chisq = 6.05) value and small p value (p = 0.014), there is significant evidence of non-proportional hazards. 

In Fig 3.1 the estimate shows a steady increase in the hazard until approximately the five month mark, then the hazard is constant until around the ten month time mark, after that it is seen to rise until the end of the treatment period. This reflects the effect seen in the KM curve, where the bevacizumab treatment is benefical to the patient compared to those treated with with a placebo, however, by the end of the treatment period the experimental treatment seems to offer no additional benefit to the patient.
...
The reported HR for the Cox Model in the bevacizumab treatment arm is: 0.87 (placebo is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the bevacizumab is 0.87 times as likely to die as a patient who is on placebo treatment. Conversely a patient on the standard of care is 1.13 times as likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the bevacizumab treatment is ((1-0.87)*100) 13% less likely to die than those patients on the placebo dose if the assumptions of the model are satisfied.  
Furthermore, the model is 95% confident that the true HR lies between 0.73 and 1.04 and the p-value for the test that the bevacizumab coefficient is zero is less than 0.12.
...
At a given instant in time a patient receiving the Nivolumab treatment is 0.852 times as likely to die as a patient who is on the Docetaxel treatment. The 95% confidence interval was (0.716, 1.015) with p value 0.073.
...
The WLRT results shown in table 3.3, reveal that only the p-value for FH(1,0) is statistically significant. As per the KM plot, the experimental treatment derives benefit for the patients throughout the course of the treatment period $t$. However, examination of the Schoenfeld plot shows that for most of the early follow up patients are benefiting from a HR < 1 this continues to around the thirteen month mark after which the HR > 1 and patients stop deriving benefit from the experimental treatment when compared to the standard of care. Therefore, it is prudent to weight early, as it is expected that most of the events would occur at the end of the study.
...
The Ipilimumab Study: titled "Ipilimumab 10 mg/kg versus ipilimumab 3 mg/kg in
patients with unresectable or metastatic melanoma" was a multinational double-blind phase 3 trial that recruited patients suffering from previously untreated or treated advanced skin cancer and who had not previously been treated with immune checkpoint or BRAF inhibitors. The subjects were randomly assigned (all participants were masked to which treatment arm they were assigned to) to receive either ipilimumab 10 mg/kg or 3 mg/kg, which was delivered intravenously every 3 weeks for 90 minutes. Furthermore, patients were stratified by disease progression status.

The primary endpoint of the trial was the progression free survival of the subject in the intend to treat population. 

From a visual inspection it is seen that the ipilimumab 10 mg/kg and the 3 mg/kg hazard are the same for the first seven months of the study. After seven months the treatment arms separate, and ipilimumab 10 mg/kg hazard is less than 3 mg/kg hazard for the remainder of the treatment period.
...
Due to the test statistic (chisq = 1.79) value and large p value (p = 0.18), there is not significant evidence of non-proportional hazards. However, from the observed KM curves there clearly show non-proportional hazards at the beginning of the time period.

In Fig 3.1 the estimate shows the hazard is constant until approximately the two-month mark, then the hazard slowly declines until around the twelve-month mark, after that it is seen to rise until the end of the treatment period. This reflects the effect seen in the KM curve, where the 10mg and 3mg treatment seem to offer the same level of benefit to the patient in the first nine months. However, by the end of the treatment period the 10mg seems to offer additional benefit to the patient compared to the 3mg dose.
...
The reported HR for the Cox Model in the ipilimumab 10 mg/kg treatment arm is: 0.85 (ipilimumab 3 mg/kg is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the ipilimumab 10 mg/kg is 0.85 times as likely to die as a patient who is on ipilimumab 3 mg/kg treatment. Conversely a patient on the standard of care is 1.15 times as likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the ipilimumab 10 mg/kg treatment is ((1-0.85)*100) 15% less likely to die than those patients on the ipilimumab 3 mg/kg dose if the assumptions of the model are satisfied.  

Furthermore, the model is 95% confident that the true HR lies between 0.72 and 1.01 and the p-value for the test that the ipilimumab 10 mg/kg coefficient is zero is less than 0.061.
...
At a given instant in time a patient receiving the Nivolumab treatment is 0.846 times as likely to die as a patient who is on the Docetaxel treatment. The 95% confidence interval was (0.691, 1.0348) with p value 0.179.
...
The OAK study 2017: was titled "Atezolizumab versus docetaxel in patients with previously treated non-small-cell lung cancer" investigated if randomly assigned cancer patients aged 18 and above and had not received some forms of prior treatment including docetaxel to receive either docetaxel 75 mg/m2 every 3 weeks or an intravenous dose of 1200mg of atezolizumab

The primary endpoint of the trial was the progression free survival of the subject in the intend to treat population and PD-L1-expression population on their cancers. 
...
From a visual inspection the atezolizumab and docetaxel hazard are the same for the first three months of the study. After three months the treatment arms separate and atezolizumab is less than and docetaxel hazard for the remainder of the treatment period.
...
Due to the test statistic (chisq = 0.717) value and large p value (p = 0.4), there is not significant evidence of non-proportional hazards. However, from the observed KM curves there clearly show non-proportional hazards at the beginning of the time period.

In Fig 3.1 the estimate shows the hazard is declining very slowly until around the nine-month mark, then the hazard slowly increases until around the twenty-two-month mark. This reflects the effect seen in the KM curve, where the atezolizumab and docetaxel treatment seem to offer the same level of benefit to the patient in the first five months. However, by the end of the treatment period the atezolizumab seems to offer additional benefit to the patient compared to the standard of care.
...
The reported HR for the Cox Model in the atezolizumab treatment arm is: 0.72 (docetaxel is the reference group). An interpretation of the HR is that; at an instant in time a patient receiving the atezolizumab is 0.72 times as likely to die as a patient who is on docetaxel treatment. Conversely a patient on the standard of care is 1.15 times as likely to experience the event as a patient on the experimental treatment.

By subtracting one from the HR, this ratio can be interpreted this as a percentage change. At a given instant a patient on the atezolizumab treatment is ((1-0.72)*100) 28% less likely to die than those patients on the docetaxel dose if the assumptions of the model are satisfied.  

Furthermore, the model is 95% confident that the true HR lies between 0.61 and 0.85 and the p-value for the test that the atezolizumab coefficient is zero is less than 0.001.
...
At a given instant in time a patient receiving the Atezolizumab treatment is 0.0.728 times as likely to die as a patient who is on the Docetaxel treatment. The 95% confidence interval was (0.614, 0.863) with p value 0.000.
