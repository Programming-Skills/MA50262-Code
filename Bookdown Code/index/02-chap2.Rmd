...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Patients with unknown EGFR mutation status"}
source("Libraries.R")
...
result <- wlr.Stat(surv=DT$time, cnsr=DT$event, trt= DT$arm, fparam=list(rho=c(0,0,1,1), gamma=c(0,1,1,0), wlr='FH(0,1)', APPLE=NULL))
...
IPD.Mok.C <- readRDS("data/IPD.Mok.A.RDS")
...
IPD.Mok.C <- readRDS("data/IPD.Brahmer.a.RDS")
...
result <- wlr.Stat(surv=DT$time, cnsr=DT$event, trt= DT$arm==0, fparam=list(rho=c(0,0,1,1), gamma=c(0,1,1,0), wlr='FH(0,1)', APPLE=NULL))
