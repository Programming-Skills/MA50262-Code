...
# readRDS(file = "IPD.Mok.A.carboplatin.RDS")

IPD.Mok.A <- rbind(IPD.Mok.A.carboplatin, IPD.Mok.A.gefitinib)

# IPD.Mok.A.time <- rbind(IPD.Mok.A.gefitinib, IPD.Mok.A.carboplatin)[,1]
# IPD.Mok.A.event <- rbind(IPD.Mok.A.gefitinib, IPD.Mok.A.carboplatin)[,2]
# IPD.Mok.A.arm <- rbind(IPD.Mok.A.gefitinib, IPD.Mok.A.carboplatin)[,3]

IPD.Mok.A <- as.data.frame(x = IPD.Mok.A)

colnames(IPD.Mok.A) <- c("time", "event", "arm")

saveRDS(IPD.Mok.A, file = "IPD.Mok.A.RDS") 

fwrite(IPD.Mok.A, file = "IPD.Mok.A.csv", sep = ",")
