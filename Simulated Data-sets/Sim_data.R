...
delayed_effect_sim = function(n_c = 165, n_e = 165, rec_period = 12, rec_power = 1, med_c = 6, rate_e_1 = log(2) / 6,
                              rate_e_2 = log(2) / 9, delay = 2, max_cal_t  = 36, n_events = NULL, model = NULL){
...
sim_data <- crossing_hazards_sim(model = model_CH)

sim_data <- as.data.frame(x = sim_data)

colnames(sim_data) <- c("time", "event", "arm")

# sim_data$event <- ifelse(sim_data$event, 1, 0)

saveRDS(sim_data, file = "~/R/Quantifying-treatment-differences-in-confirmatory-trials-under-non-proportional-hazards/crossing_hazards_sim_data.RDS", ) 

fwrite(sim_data, file = "~/R/Quantifying-treatment-differences-in-confirmatory-trials-under-non-proportional-hazards/crossing_hazards_sim_data.csv", sep = ",")

fit<- survfit(Surv(time, event) ~ arm, data = sim_data)

ggsurv <- ggsurvplot(
  fit,                     # survfit object with calculated statistics.
  data = sim_data,             # data used to fit survival curves.
...
ggsave("~/R/Quantifying-treatment-differences-in-confirmatory-trials-under-non-proportional-hazards/Figure_3.png",ggsurv, width = 10, height = 4, dpi = 300)
