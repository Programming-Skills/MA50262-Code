...
plot_dt <- data.table(event = rep(result$events, 2),
                      power = c(result$power_logrank_DH, result$power_logrank_PH),
                      test = rep(c("Diminishing Treatment Effect", "Proportional Hazards"), c(n, n)))

plot_dt$test = factor(plot_dt$test,levels = c("Proportional Hazards","Diminishing Treatment Effect"), labels = c("Proportional Hazards","Diminishing Treatment Effect"),ordered = TRUE)

p <- ggplot(aes(x = event, y = power, color = test), data = plot_dt) +
  geom_vline(xintercept = 258, linetype="dashed", color = "black", size=1) +
  geom_line(size = 1.5) +
  ggtitle(label = "Number of Events vs Power") +
  scale_y_continuous(labels = function(x) paste0(x*100, "%"), limits = c(0,1)) +
  scale_x_continuous("Number of events") +
  scale_color_manual(values=c("#bae1ff","#baffc9")) +
  labs(y = "Power", color = "") +
  theme_light() +
