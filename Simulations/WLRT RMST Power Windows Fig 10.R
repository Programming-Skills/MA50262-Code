...
sim_data <- decreasing_hazards_sim(model = model_DH)
...
p1 = p$plot + geom_segment(aes(x = rt, y = 0, xend = rt, yend = 0.5),color = "grey", size = 0.3) +
  ...
  annotate("text", x = 15, y = 0.55, label = "Window LRT") +
...
df$scenario = factor(df$scenario,levels = c("Proportional Hazards","Delayed Effects","Decreasing Effects","Crossing Hazards"), labels = c("Proportional Hazards","Delayed Treatment Effect","Diminishing Treatment Effect","Crossing Hazards"),ordered = TRUE)

p2 <- ggplot(aes(x = time, y = power, color = scenario), data = df) +
  geom_line(size = 1.2) +
  scale_x_continuous(expression(paste("Cutting time ","(",t['cut'],")"))) +
  scale_y_continuous(labels = function(x) paste0(x*100, "%"), limits = c(0,1)) +
  labs(y = "Power", color = "",title = "B)") +
  scale_color_manual(values=c("#bae1ff", "#baffc9", "#ffdfba", "#ffb3ba")) +
  theme_light() +
